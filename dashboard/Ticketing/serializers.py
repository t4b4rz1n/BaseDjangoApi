...
from panel.Ticketing.models import Ticket, Message, Attachment, TicketType
...
class MessageCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_attachments(self, files):
        if len(files) > MAX_ATTACHMENTS_PER_MESSAGE:
            raise serializers.ValidationError(f"You can upload a maximum of {MAX_ATTACHMENTS_PER_MESSAGE} files.")
        for file in files:
            if file.size > MAX_FILE_SIZE:
                raise serializers.ValidationError(f"File size exceeds the limit of {MAX_FILE_SIZE // 1024 // 1024}MB.")
        return files
...
class TicketListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Ticket
        fields = ['id', 'title', 'user', 'ticket_type', 'priority', 'status', 'created_at', 'updated_at']
...
class TicketDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Ticket
        fields = ['id', 'title', 'user', 'ticket_type', 'priority', 'status', 'created_at', 'updated_at', 'closed_at']
